...
def get_user_preferences(context=None):
    ...
    # Ambil nama folder paling depan saja (QuickTools)
    # Ini supaya kalau file ini ada di dalam folder 'utils', dia tetep dapet nama 'QuickTools'
    ...
    elif hasattr(context, "user_preferences"):
...
def update_settings_ui(self, context, element=None):
    ...
    if updater.error is not None and updater.error_msg is not None:
        ...
        row.label(text="Terakhir cek update: " + last_check)
    else:
        row.label(text="Terakhir cek update: belon pernah")
...
def update_settings_ui_condensed(self, context, element=None):
    ...
    if not updater.auto_reload_post_update:
        ...
        if "just_updated" in saved_state and saved_state["just_updated"]:
            ...
            row.operator(
                "wm.quit_blender",
                text="Restart blender buat menyelesaikan updatenya bro",
                icon="ERROR")
            return
    ...
    if updater.error is not None and updater.error_msg is not None:
        ...
        #last_check = last_check[0: last_check.index(".")]
        row.label(text="Terakhir cek: " + last_check)
    else:
        row.label(text="Terakhir cek: belon pernah")
...
def register(bl_info):
    ...
    updater.user = "projectextt"

    # Choose your own repository, must match git name for GitHUb and Bitbucket,
    # for GitLab use project ID (numbers only).
    updater.repo = "QuickTools"
    updater.zip_name = "QuickTools.zip"
    
    
    # updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but recommended to set.
    updater.website = "https://github.com/projectextt/QuickTools"
    ...
    updater.overwrite_patterns = ["*.py", "*.pyc", "*.png", "*.jpg", "README.md", "LICENSE.txt"]
    ...
    updater.remove_pre_update_patterns = ["*.py", "*.pyc", "__pycache__"]
